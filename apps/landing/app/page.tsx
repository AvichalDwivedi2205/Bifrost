...
const GITHUB = 'https://github.com/AvichalDwivedi2205/Bifrost';
import {
  motion,
  useScroll,
  useTransform,
...
// ── CTASection ─────────────────────────────────────────────────────────────────
function CTASection({ onLaunch: _onLaunch, onRegistry: _onRegistry }: { onLaunch: () => void; onRegistry: () => void }) {
  return (
    <section style={{ position: 'relative', padding: '120px 60px', textAlign: 'center', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <Reveal style={{ position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 48 }}>
          <Btn variant="primary" size="lg" icon="bolt" onClick={() => window.open(GITHUB, '_blank')}>View on GitHub</Btn>
          <Btn variant="default" size="lg" onClick={() => window.open(GITHUB + '/issues', '_blank')}>Contribute →</Btn>
        </div>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', position: 'relative' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Btn variant="ghost" size="sm" onClick={() => window.open(GITHUB, '_blank')}>
            GitHub
          </Btn>
          <Btn variant="primary" size="sm" onClick={() => window.open(GITHUB, '_blank')} icon="arrow">
            View source
          </Btn>
        </div>
      </header>
        ...
        <div style={{ position: 'relative', width: '100%', maxWidth: 1360, margin: '0 auto', padding: '0 40px', display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 48, alignItems: 'center' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <Btn variant="primary" size="lg" onClick={() => window.open(GITHUB, '_blank')} icon="bolt">
                View on GitHub
              </Btn>
              <a href="#how" style={{ textDecoration: 'none' }}>
                <Btn variant="default" size="lg">
                  See how it works →
                </Btn>
              </a>
            </div>
      ...
      {/* CTA */}
      <CTASection onLaunch={() => {}} onRegistry={() => {}} />
